...
def parse_args():
    ...
    parser.add_argument(
        "--model_type", type=str, default="vicuna", help="The type of LLM"
    )
    parser.add_argument(
        "--options",
        nargs="+",
...
def upload_imgorvideo(gr_video, gr_img, text_input, chat_state, chatbot, audio_flag):
    ...
    if gr_img is None and gr_video is None:
        ...
        return (
            gr.update(interactive=False),
            gr.update(
                interactive=False, placeholder="Currently, only one input is supported"
            ),
            gr.update(
                value="Currently, only one input is supported", interactive=False
            ),
            chat_state,
            None,
            chatbot,
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            text_input = gr.Textbox(
                label="User",
                placeholder="Upload your image/video first, or directly click the examples at the bottom of the page.",
                interactive=False,
            )
    ...
    with gr.Column():
        gr.Examples(
            examples=[
                ...
                [
                    f"examples/Statue_of_Liberty.jpg",
    ...
    upload_button.click(
        upload_imgorvideo,
        [video, image, text_input, chat_state, chatbot, audio],
        [video, image, text_input, upload_button, chat_state, img_list, chatbot],
    )
    ...
    clear.click(
        gradio_reset,
        [chat_state, img_list],
        [chatbot, video, image, text_input, upload_button, chat_state, img_list],
        queue=False,
    )
