...
if __name__ == "__main__":
    ...
    tokenizer = LanguageBindImageTokenizer.from_pretrained(
        pretrained_ckpt, cache_dir="./cache_dir/tokenizer_cache_dir"
    )
    modality_transform = {
        c: transform_dict[c](model.modality_config[c]) for c in clip_type
    }
    ...
    with gr.Blocks(title="LanguageBind🚀", css=css) as demo:
        ...
        with gr.Row():
            with gr.Column():
                depth = gr.File(
                    label="Depth Input, need a .png file, 16 bit, with values ranging from 0-10000 (representing 0-10 metres, but 1000 times)",
                )
            ...
            with gr.Column():
                thermal = gr.Image(
                    type="filepath",
                    label="Thermal Input, you should first convert to RGB",
                )
